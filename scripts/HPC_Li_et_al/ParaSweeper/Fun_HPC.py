...
def Get_Values_Excel(
    ...
    if model_options.__contains__("SEI on cracks"):
        LossCap_seioncrack = my_dict_RPT["CDend LLI SEI on cracks [%]"][-1]
    else:
        LossCap_seioncrack = "nan"
    if model_options.__contains__("lithium plating"):
        LossCap_LiP = my_dict_RPT["CDend LLI lithium plating [%]"][-1]
    else:
        LossCap_LiP = "nan"
    if model_options.__contains__("SEI"):
        LossCap_SEI = my_dict_RPT["CDend LLI SEI [%]"][-1]
    else:
        LossCap_SEI = "nan"
    if DryOut == "On":
    ...
    values_pos = [
        ...
        str(my_dict_RPT["CDend LLI [%]"][-1]),
        str(LossCap_LiP),
        str(LossCap_SEI),
        str(LossCap_seioncrack),
        str(my_dict_RPT["CDend LLI due to LAM [%]"][-1]),
        str(my_dict_RPT["LAM_to_Crack_NE [%] end"]),
        str(my_dict_RPT["LAM_to_Crack_PE [%] end"]),
        str(my_dict_RPT["LAM_to_Dry [%] end"] ),
...
def Plot_DMA_Dec(my_dict_RPT,Scan_i,Temper_i,model_options,
    ...
    plt.savefig(
        ...
        f"0_Scan_{Scan_i}-{str(int(Temper_i- 273.15))}degC LLI break down.png", dpi=dpi)
    ...
    plt.savefig(
        BasicPath + Target+    "Plots/" +  
        f"0_Scan_{Scan_i}-{str(int(Temper_i- 273.15))}degC LAM break down.png", dpi=dpi)
    plt.close()  # close the figure to save RAM
    return
...
def Get_SOH_LLI_LAM(my_dict_RPT,model_options,DryOut,mdic_dry):
    my_dict_RPT['Throughput capacity [kA.h]'] = (
    ...
    if DryOut == "On":
        ...
        LAM_to_Dry_i = 0
    LAM_to_Crack_NE_i=my_dict_RPT['CDend LAM_ne [%]'][-1]-LAM_to_Dry_i
    LAM_to_Crack_PE_i=my_dict_RPT['CDend LAM_pe [%]'][-1]-LAM_to_Dry_i
    my_dict_RPT["LAM_to_Dry [%] end"] = LAM_to_Dry_i
    my_dict_RPT["LAM_to_Crack_NE [%] end"] = LAM_to_Crack_NE_i
    my_dict_RPT["LAM_to_Crack_PE [%] end"] = LAM_to_Crack_PE_i
    return my_dict_RPT
...
def RunOneCyc(Para_dict_i,BasicPath, XY_Exp, 
    ...
    Experiment_OneCyc   = pb.Experiment( exp_1Cyc_text * 1  )  
...
def Run_Age(
    ...
    if Runshort == False:
        ...
        Total_Cycles = 2; Cycle_bt_RPT = 1; Update_Cycles = 1
    ...
    Experiment_Breakin= pb.Experiment( exp_breakin_text*1 )
    ...
    try:  
        ...
        if Call_Breakin.success == False:
            Sol_0 = "nan"
            call_logs = str(Call_Breakin.logs)
            own_logger.info("Fail due to Experiment error or infeasible")
            1/0
        if Sol_0 == Timeout_text: # to do: distinguish different failure cases
            own_logger.info("Fail due to Timeout")
    ...
    if Flag_Breakin == False: 
        ...
        return midc_merge,Sol_RPT,Sol_AGE,Call_Breakin
    ##########################################################
    ##############   Part-3: Post-prosessing    ##############
    ##########################################################
    ...
    # Niall_data = loadmat( 'Extracted_all_cell.mat'
        ...
        my_dict_RPT = Get_SOH_LLI_LAM(my_dict_RPT,model_options,DryOut,mdic_dry)
        ...
        Plot_DMA_Dec(my_dict_RPT,Scan_i,Temper_i,model_options,
            BasicPath, Target,fs,dpi)
        ...
        with open(
            ...
            + str(Scan_i)+ '-midc_merge.pkl', 'wb') as file:
...
def Get_SALib_Sample(
        BasicPath,Target_name,Para_dict, seed,
        num,rows_per_file,Bundle=True):

    param_bounds = []
    param_samples = []
    ParaDict_unchange ={};  Vars = []
    for key, value in Para_dict.items():
        # depending on what values is:
        # it is a list, then it is changing
        if isinstance(value,list):
            if len(value) >1:
                #print(value)
                raise ValueError("No more support list with multiple values")
            else:
                ParaDict_unchange[key] = value[0]
        # it is a list, so it also changes
        elif isinstance(value,tuple):
    ...
    seed = 42  # Set your desired random seed, important to ensure reproducibility 
    np.random.seed(seed)
    param_samples = latin.sample(problem, num) # desired number of samples


    Big_Comb = []
    if len(param_samples) > 0:  
        for comb_tu in param_samples:
            Big_Comb.append(comb_tu)
    else:
        pass
    # Print or do something with Big_Comb
...
# directly get single excel into the biggest one to aviod trouble!
def Collect_single_to_Big(Scan_start_all,Scan_end_all,i_succ,BasicPath,excel_name,sheet_title,purpose_i):
    data_tar = openpyxl.load_workbook(BasicPath+f"/{excel_name}.xlsx" )
    target_sheet = data_tar[sheet_title]

    # get the first row:
    Target  = f'/{purpose_i}_Case_{Scan_start_all[i_succ]}_{Scan_end_all[i_succ]}/'  
    purpose = f"{purpose_i}_Case_{Scan_start_all[i_succ]}_{Scan_end_all[i_succ]}"
    book_name_xlsx =  f'Summary_{purpose}.xlsx'
    first_row_wb =  openpyxl.load_workbook(
        filename = BasicPath +Target+ book_name_xlsx)
    first_row_sheet = first_row_wb.active
    source_row = next(first_row_sheet.iter_rows(values_only=True))
    target_sheet.append(source_row)

    for Scan_start,Scan_end in zip(Scan_start_all,Scan_end_all):
        # get the first row of title:
        # input: purpose_i, excel_name
        Index_List_succeed = np.arange(Scan_start-1,Scan_end)+1
        ...
        for k,index_i in enumerate(Index_List_succeed):
            try:
                ...
                print(f"Successfuly write results for Scan {index_i}!") 
    data_tar.save(BasicPath+f"/{excel_name}.xlsx")
    return 
