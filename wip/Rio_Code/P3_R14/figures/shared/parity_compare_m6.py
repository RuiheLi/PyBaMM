"""Compare parity_source_m6.npz and parity_port_m6.npz."""
import os
import sys
...
SRC = os.path.join(os.path.dirname(__file__), "parity_source_m6.npz")
PORT = r"D:/LRHWork/Model_RH/PyBaMM-port-work/wip/parity_port_m6.npz"
...
def main():
    ...
    print(f"  source: c_EC[0]={cEC_s[0]:.4f}, c_EC[-1]={cEC_s[-1]:.4f}, drop={cEC_s[0]-cEC_s[-1]:+.4f}")
    print(f"  port:   c_EC[0]={cEC_q[0]:.4f}, c_EC[-1]={cEC_q[-1]:.4f}, drop={cEC_q[0]-cEC_q[-1]:+.4f}")
