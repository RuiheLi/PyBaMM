"""Compare parity_source_m8.npz and parity_port_m8.npz."""
import os
import sys
...
SRC = os.path.join(os.path.dirname(__file__), "parity_source_m8.npz")
PORT = r"D:/LRHWork/Model_RH/PyBaMM-port-work/wip/parity_port_m8.npz"
...
def main():
    ...
    print(f"  source: Q_sei[-1]={Q_s[-1]:.4e}")
    print(f"  port:   Q_sei[-1]={Q_q[-1]:.4e}")
    print(f"  ratio (port/source): {Q_q[-1]/Q_s[-1]:.2f}x")
    print("  NB: the absolute Q_sei differs because the source's v23-era and "
          "the port's v24 'reaction limited' SEI use slightly different "
          "kinetic forms / initial conditions. The c_EC and voltage agreement "
          "is the meaningful M8 test, and it shows the dimensional source "
          "terms (consumption + refill) are wired identically.")
    return 0
